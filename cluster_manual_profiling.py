...
for N in Ns:
    ...
    accepted_proposals = proposals['proposals'][-N:] # TEMP
    ...
    print('\n.....\ndata preparation time pre-clustering (s):' + str(np.round((new_time := time.time()) - time_last,3)) + '\n.....\n', flush = True)
    ...
    for k in clusters_counts:
        ...
        if verbosity > 0 or True:
            print('\n.....\ndone k = ' + str(k) + '\nthis k time (s):' + str(temp := (np.round((new_time := time.time()) - time_last,2))) 
                  + '\n.....\n', flush = True)
            time_last = new_time
            
        times_this_N.append(temp)
        
    times.append(times_this_N)
    ...
    if False: # SSE and SS vs k plots:
...
# matrix plot of stacked profiling times, 1st axis is N, 2nd axis is k
#%%
M = np.stack(times, axis = 1)
np.savetxt(output_name + 'clustering_profiling.txt', M, delimiter = ',') # , fmt='%.3f')
plt.figure(tight_layout = True)
plt.matshow(M)
plt.xlabel(r'$N$')
plt.ylabel(r'k')
...
plt.colorbar(fraction = 0.1, label = 'time (s)')
plt.savefig(output_name + 'clustering_profiling.svg')
